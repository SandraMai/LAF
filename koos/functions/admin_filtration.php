...
function displayLostItemsFiltrationAdmin($offset,$searchedName,$searchedCategory,$thisLink){
    if($searchedName==null && $searchedCategory==null){
        ...
        // if($filter == null){
        //     $stmt = $conn->prepare("SELECT lost_post_ID, description, picture, DATE_FORMAT(lost_date, '%d'), DATE_FORMAT(lost_date, '%c'), DATE_FORMAT(lost_date, '%Y') 
        ...
        //}
        echo $conn->error;
        $stmt->bind_result($id, $description, $pic, $day, $month, $year);
        $stmt->execute();
